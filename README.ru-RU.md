...
<p align="center">
Возможно, первая в мире платформа, где каждый может подключить свой токен к P2P-сети и взамен использовать токены, расшаренные другими пирами. Без центрального сервера, без регистрации аккаунта, ни один API-ключ никогда не покинет вашу машину.
</p>

> ⚠️ **Статус проекта (v1 тест)**: ModelBus-P2P всё ещё в разработке и публичном тестировании. Компоненты wire-формата (протокол `/modelbus/inference/1.0.0`, схема v2 объявлений узлов, список корней доверия) могут получать несовместимые обновления в будущем; пиры, объявленные под старыми версиями, могут нуждаться в повторной регистрации.
...
- [节点公告格式（v2）](#节点公告格式v2)
- [调用流程详解](#调用流程详解)
- [Загрузка и использование (скоро)](#загрузка-и-использование-скоро)
- [目录结构](#目录结构)
- [快速开始](#快速开始)
- [配置说明](#配置说明)
- [开发与调试](#开发与调试)
