...
<p align="center">
或許全球首個，任何人都可以把自己的 Token 掛上 P2P 網路，也可以因此呼叫網路上更多其他節點共享的 Token。無需中心伺服器、無需註冊帳號、不會丟失任何 API Key。
</p>

> ⚠️ **專案狀態（v1 測試版）**：ModelBus-P2P 仍在開發與公開測試階段。協定格式（`/modelbus/inference/1.0.0`、節點公告 v2 schema、信任根清單）未來可能出現不向下相容的升級；早期 announce 的節點在新版本上線後可能需要重新註冊。
